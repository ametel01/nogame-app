...
import fetchUpgradesLevels from "../services/fetchUpgradesLeveles";

export const getUpgradesLevels = async (req: Request, res: Response) => {
  ...
  try {
    const data = await fetchUpgradesLevels(planetId);
    res.json(data);
  } catch (error) {
...
export default getUpgradesLevels;
